...
from packages_simulations.class_GRB import GRB
...
def simulate_observation(template_grb, z, imx, imy, ndets, resp_mat, sim_triggers=False,ndet_max=32768,bgd_rate_per_det=0.3):
	...
	synth_GRB.light_curve *= det_frac
	...
	synth_GRB.light_curve['RATE'] = rate_15_350keV * dt
...
def apply_mask_weighting(light_curve,imx,imy,ndets):
	"""
	Method to apply mask weighting to a light curve.
	Mask-weighted means:
	...
	# Rough calculation of the background
	background = np.mean(light_curve["RATE"][0:20]) 
	stdev_backgroud = np.std(light_curve["RATE"][0:20])

	# Calculate the mask-weighted RATE column
	light_curve['RATE'] = (light_curve["RATE"] - background)/correction
