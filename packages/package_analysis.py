	...
	for i in range(len(param_list)):
		...
		if multiproc is False:
			...
			for j in range(trials):
				...
				sim_results[sim_result_ind] = bayesian_blocks(synth_GRB) # Find the T90 and the fluence 
			...
			for k in range(num_cores):
				sim_results[sim_result_ind] = bayesian_blocks(t=synth_GRBs[k].light_curve['TIME'],x=synth_GRBs[k].light_curve['RATE'],sigma=synth_GRBs[k].light_curve['UNC'],fitness="measures")
				sim_result_ind += k
